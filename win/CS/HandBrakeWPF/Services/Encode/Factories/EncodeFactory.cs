...
namespace HandBrakeWPF.Services.Encode.Factories
{
    ...
    internal class EncodeFactory
    {
        ...
        private static Filters CreateFilters(EncodeTask job)
        {
            ...
            {
                IntPtr settingsPtr = HBFunctions.hb_generate_filter_settings((int)hb_filter_ids.HB_FILTER_DEINTERLACE, job.Deinterlace.ShortName, job.CustomDeinterlace);
            ...
            {
                IntPtr settingsPtr = HBFunctions.hb_generate_filter_settings((int)hb_filter_ids.HB_FILTER_DECOMB, job.Decomb.ShortName, job.CustomDecomb);
                string settings = Marshal.PtrToStringAnsi(settingsPtr);
            ...
            {
                IntPtr settingsPtr = HBFunctions.hb_generate_filter_settings((int)hb_filter_ids.HB_FILTER_ROTATE, string.Format("{0}:{1}", job.Rotation, job.FlipVideo ? "1" : "0"), IntPtr.Zero);
                string settings = Marshal.PtrToStringAnsi(settingsPtr);
